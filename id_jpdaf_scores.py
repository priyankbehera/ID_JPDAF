...
# coding: utf-8
"""
Compute accuracy (RMSE) for StoneSoupID’s Influence-Diagram JPDAF
...
from StoneSoupID.stonesoupID.dataassociator.probability import JPDA
    ...
    for k in range(steps):
        ...
        for truth in truths:
            if np.random.rand()<=pd:
                ...
                M.add(TrueDetection(z, truth, t, meas_model))
        for truth in truths:
            ...
            for _ in range(np.random.poisson(clutter*100)):
                ...
                M.add(Clutter(np.array([[x],[y]]),t,meas_model))
        all_meas.append(M)
    return truths, all_meas

def run_idjpda_once(seed, clutter, pd, noise_var):
    np.random.seed(seed)
    start = datetime.now()
    ...
    for t, M in enumerate(measurements):
        ...
        hyps = assoc.associate(tracks, M, time)
        for i,tr in enumerate(tracks):
            ...
            tr.append(GaussianState(pm.reshape(-1,1),None,timestamp=time))
    return rmse
        ...
        for pd in p_grid:
            for nv in noise_grid:
                ...
                for s in range(Ntrials):
                    r=run_idjpda_once(s,cd,pd,nv)
                    all_rmse.append(r.flatten())
                A=np.stack(all_rmse,axis=0)
                print(f"{cd},{pd},{nv},{A.mean():.3f},{A.std():.3f}")
