...
def simulate_scenario(clutter_density, prob_detect, meas_noise_var, start_time, steps=20):
    ...
    meas_model = LinearGaussian(
        ndim_state=4, mapping=(0, 2), noise_covar=np.eye(2) * meas_noise_var
    )
...
def run_single_trial(seed, clutter_density, prob_detect, meas_noise_var):
    ...
    truths, all_measurements = simulate_scenario(
        clutter_density, prob_detect, meas_noise_var, start_time
    )
    ...
    priors = [
        GaussianState([[0], [1], [0], [1]], np.diag([1.5, 0.5, 1.5, 0.5]), timestamp=start_time),
    ...
    for t, meas_set in enumerate(all_measurements):
        ...
        for i, tr in enumerate(std_tracks):
            hs = hyps[tr]
            # missed and false counts
            std_miss[i, t] = sum(1 for h in hs if h.measurement is None)
            std_false[i, t] = sum(
                1 for h in hs if h.measurement is not None and h.probability < 0.01
            )

            # generate posterior components and weights
            posts, ws = [], []
            for h in hs:
            ...
            means = np.stack([p.state_vector.reshape(-1) for p in posts], axis=1)
            covs = np.stack([p.covar for p in posts], axis=2)
            pm, Pm = gm_reduce_single(means, covs, np.array(ws))
            pm = np.asarray(pm).reshape(-1)  # ensure 1D vector

            # append to track
            tr.append(GaussianState(pm.reshape(-1, 1), Pm, timestamp=time))

            # compute errors
            true_state = truths[i].states[t].state_vector.ravel()
            err_vec = pm - true_state              # shape (4,)
            std_RMSE[i, t] = np.linalg.norm(err_vec)
            sol = np.linalg.solve(Pm, err_vec)      # shape (4,)
            std_NEES[i, t] = float(np.dot(err_vec, sol))
    ...
    for cd in clutter:
        for pd in pdets:
            for nv in noises:
                all_rmse, all_nees = [], []
                for seed in range(N):
                    rmse, nees, _, _ = run_single_trial(seed, cd, pd, nv)
                    all_rmse.append(rmse)
                    all_nees.append(nees)
                key = (cd, pd, nv)
                results[key] = {
                    'std_RMSE_mean': np.mean(all_rmse, axis=0),
                    'std_RMSE_std': np.std(all_rmse, axis=0),
                    'std_NEES_mean': np.mean(all_nees, axis=0),
                }
    return results
